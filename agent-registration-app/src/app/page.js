...
export default function AgentRegistrationTool() {
  ...
  const [agents, setAgents] = useState(() => { try { const d = JSON.parse(localStorage.getItem("agent_reg_data")); return d?.agents || []; } catch { return []; } });
  const [forms, setForms] = useState(() => { try { const d = JSON.parse(localStorage.getItem("agent_reg_data")); return d?.forms || []; } catch { return []; } });
  const [tab, setTab] = useState("agents"); // agents | forms | generate
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                {pdfLoading ? "Loading PDF engine..." : pdfLib ? "Ready" : "PDF engine unavailable"}
